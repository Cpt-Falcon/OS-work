 ...
 * @Date: 3/3/2016
 * @Description: minget.c is code that attempts to read
 ...
 * indirect block and uses that to print the directory. The
 * only difference between minget.c and minls.c is that 
 * minget can output file contents to standard out or to
...
void getZone(DataStore *mainStruct)
{
   ...
   if (mainStruct->hasDest == 1)
   {
      output = fopen(mainStruct->pList.destPath, "w");
      if (output == NULL)
      {
         perror("Cannot open destination file");
         exit(EXIT_FAILURE);
      }
      err = fwrite(block, mainStruct->node.size, 1, output);
      if (err < 0)
      {
         perror("write error: ");
         exit(EXIT_FAILURE);
...
void goToPath(DataStore *mainStruct)
{
   ...
   if (permission[0] == 'd')
   {
      printf("Trying to get a directory\n");
      exit(EXIT_FAILURE);
   }
}
...
int main(int argc, char *argv[]) {
   ...
   /*image, file path, and destination specified*/
   else
   {
      mainStruct.pList.imgPath = argv[argc - 3];
      mainStruct.pList.filePath = argv[argc - 2];
      mainStruct.pList.destPath = argv[argc - 1];
      mainStruct.hasPath = 1;
      mainStruct.hasDest = 1;
   }
